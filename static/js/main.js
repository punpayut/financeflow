...
function renderStockBar() {
    ...
    const stockHTML = Object.values(stockData).map(stock => {
        ...
        return `
            <div class="stock-box">
                ...
                <span class="change ${changeClass}">${sign}${stock.change.toFixed(2)} (${sign}${stock.percent_change.toFixed(2)}%)</span>
            </div>
        `;
    }).join('');
...
function renderNewsFeed() {
    if (newsData.length === 0) {
        renderError('No news available at the moment.');
        return;
    }
...
function renderNewsCard(item) {
    ...
    const publishedDate = new Date(item.published).toLocaleString();
    ...
    return `
        <div class="story-card">
            ...
            <p class="card-summary">${analysis.summary_th}</p>
            <footer class="card-footer">
                <div class="sentiment-badge ${sentimentClass}">${analysis.sentiment}</div>
                <div class="impact-score">Impact: ${analysis.impact_score}/10</div>
